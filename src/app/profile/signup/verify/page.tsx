...
export default function VerifyEmailPage() {
    ...
    // Debug logging
    console.log('VerifyEmailPage - otp state:', otp, 'user:', user, 'authLoading:', authLoading);

    // Single Responsibility: Handle authentication redirect
    useEffect(() => {
        if (!authLoading && !user) {
            router.replace('/profile/signup');
        } else if (!authLoading && user?.email_verified) {
            router.replace('/profile/signup/success');
        }
    }, [authLoading, user, router]);

    // Single Responsibility: Auto-send verification email on page load
    useEffect(() => {
        if (user && !user.email_verified && !isSendingCode) {
            handleSendVerification();
        }
    }, [user]);

    // Single Responsibility: Handle close/back action
    const handleClose = () => {
        router.back();
    };
        ...
        try {
            ...
            if (data.success) {
                ...
                setAttemptsRemaining(
                    data.error?.details?.attemptsRemaining || null
                );
