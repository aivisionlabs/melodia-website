...
import { getSongFromSession, clearSongFromSession } from '@/lib/song-session-storage'
...
export default function GeneratedSongsPage() {
  ...
  useEffect(() => {
    ...
    if (sessionData) {
      ...
      const recentSongs = savedSongs.filter((song: any) => 
        song.recipientName === recipient_name && 
        new Date(song.createdAt).getTime() > Date.now() - 60000 // Last minute
      )
      
      if (recentSongs.length > 0) {
        // Use recently generated songs
        setSongs(recentSongs.map((song: any) => ({
          id: song.id,
          title: song.title,
          lyrics: song.lyrics,
  ...
  const generateSongOnce = async (title: string, lyrics: string, style: string, recipient_name: string, songs: SongData[]) => {
    try {
      ...
      setTimeout(async () => {
        try {
          const statusResponse = await fetch(`/api/song-status/${result.taskId}`)
          const statusResult = await statusResponse.json()

          if (statusResult.status === 'completed' && statusResult.variants && statusResult.variants.length > 0) {
            // Update both songs with real audio URLs from Suno variants
            const updatedSongs = songs.map((song, index) => ({
              ...song,
              status: 'ready' as const,
              audioUrl: statusResult.variants[index]?.audioUrl || statusResult.variants[index]?.streamAudioUrl || '/audio/har-lamha-naya.mp3'
            }))
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center">
        <Card className="w-full max-w-md mx-4 bg-gray-800/50 backdrop-blur-sm border-yellow-500/30">
          <CardContent className="p-6 text-center">
            ...
            <h2 className="text-xl font-bold mb-2 text-white">No Songs Found</h2>
            <p className="text-gray-300 mb-4">Please generate songs first</p>
            <Button onClick={handleBack} className="w-full bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-black font-bold">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-center">
              ...
              <p className="text-gray-300">Your personalized songs are being created!</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Generating Message */}
        {songs.length > 0 && songs.some(song => song.status === 'generating') && (
          <Card className="mb-8 bg-gradient-to-r from-yellow-500 to-orange-500 border-0">
            <CardContent className="p-4 text-center">
              <div className="flex items-center justify-center space-x-2">
                <Music className="w-5 h-5" />
                <span className="font-semibold text-white">
                  Generating your personalized songs... This may take a few minutes.
                </span>
              </div>
            </CardContent>
          </Card>
            ...
            <Card key={song.id} className="bg-gray-800/50 backdrop-blur-sm border-yellow-500/30 hover:bg-yellow-500/10 transition-all duration-300">
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  >
                    {song.status === 'generating' ? 'Generating...' : song.status === 'ready' ? 'Ready' : 'Error'}
                  </Badge>
                </div>
              </CardHeader>
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[80vh] overflow-hidden">
            ...
            <div className="p-4 max-h-96 overflow-y-auto">
              <div className="bg-gray-50 rounded-lg p-4 mb-4">
                ...
                <p className="text-sm text-gray-600">{selectedSong.styleOfMusic}</p>
              </div>
              ...
              <div className="mt-4 flex space-x-2">
                {selectedSong.audioUrl ? (
                  <Button
                    onClick={() => {
                      if (currentlyPlaying && playingSongId === selectedSong.id) {
              ...
              <div className="mt-2 p-3 bg-blue-100 border border-blue-300 rounded-lg">
                <p className="text-sm text-blue-800">
                  <strong>Note:</strong> This is demo audio. Your actual lyrics are displayed above.
                </p>
              </div>
            </div>
          </div>
