...
import {
  ...
  getLyricsDraftsAction,
} from "@/lib/lyrics-actions";
import { SongRequest, LyricsDraft } from "@/types";

export default function CreateSongFromLyricsPage({
  params,
}: {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Get the approved lyrics draft (approved_lyrics_id moved to songs table)
      const drafts = await getLyricsDraftsAction(parseInt(requestId));
      const approvedDraft = drafts.find(
        (draft: LyricsDraft) => draft.status === "approved"
      );
            ...
            <div className="p-8">
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-medium mb-3 text-gray-700">
                    Tempo (BPM):{" "}
                    <span className="text-yellow-600 font-semibold">{bpm}</span>
                  </label>
                  <input
                    type="range"
                    min="60"
