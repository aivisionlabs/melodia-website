...
function LyricsDisplayContent() {
  ...
  const [songGenerationError, setSongGenerationError] = useState<string | null>(
    null
  );
      ...
      if (requestId) {
        ...
        try {
          ...
          if (data) {
            // Set lyrics data
            setLyricsData({
              title: `Song for ${data.songRequest.recipient_details}`,
              styleOfMusic: "Personal", // Default style
              lyrics: data.lyricsDraft.generated_text,
            });
            setEditedStyle("Personal");

            // Set form data
            setFormData({
              recipient_name: data.songRequest.recipient_details,
        ...
        if (recipient_name && languages.length > 0 && additional_details) {
          setFormData({ recipient_name, languages });
        }
      }
    };
  ...
  const handleGenerateSong = async () => {
    ...
    if (checkSongLimit()) {
      addToast({
        ...
        message:
          "You have reached the maximum limit of 5 songs. Please delete some songs from My Songs to create new ones.",
      });
      return;
    }
  ...
  const pollSongStatusOnce = async (taskId: string, songs: GeneratedSong[]) => {
    try {
      // Check if this is a demo task ID
      const isDemoTask = taskId.startsWith("demo-task-");

      if (isDemoTask) {
        // For demo tasks, wait 2 minutes then mark as completed with mock audio
        console.log(
          "🎭 Demo task detected, waiting 2 minutes for completion simulation"
        );
        ...
        const updatedSongs = songs.map((song) => ({
          ...
          audioUrl: "/audio/har-lamha-naya.mp3", // Demo audio
        }));
        ...
        addToast({
          type: "success",
          title: "Demo Songs Ready!",
          message: "Your demo songs have been generated successfully!",
        });
        return;
      }

      // For real Suno tasks, wait 30 seconds before checking
      ...
      if (
        ...
        const updatedSongs = songs.map((song, index) => ({
          ...
          status: "ready" as const,
          audioUrl:
            result.variants[index]?.audioUrl ||
            result.variants[index]?.streamAudioUrl ||
        ...
        addToast({
          ...
          message:
            "Your songs are still processing. Playing demo versions for now.",
        });
      }
    } catch (error) {
  ...
  const handleGenerateNewLyrics = async () => {
    ...
    try {
      ...
      const additionalContext = modificationRequest.trim()
        ? `Style: ${lyricsData.styleOfMusic}. Modification request: ${modificationRequest}`
        : `Style: ${lyricsData.styleOfMusic}`;
      ...
      if (lyricsResult.error) {
        alert(
          `Error: ${lyricsResult.message || "Failed to generate new lyrics"}`
        );
        return;
      }
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-foreground text-center">
          ...
          <p className="text-muted-foreground">
            Please wait while we prepare your personalized lyrics.
          </p>
        </div>
      </div>
    );
            ...
            <Card className="bg-card border border-melodia-teal-medium">
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <p className="text-card-foreground text-lg mt-1 font-semibold">
                ...
                <CardContent>
                  <div className="space-y-4">
                    {generatedSongs.map((song) => (
                      <div
                        key={song.id}
                        className="bg-secondary rounded-xl p-4 border border-melodia-teal-medium"
                      >
                              ...
                              {song.status === "error" && (
                                <div className="flex items-center text-destructive">
                                  ...
                                  <span className="text-sm">
                                    {song.errorMessage || "Generation failed"}
                                  </span>
                                </div>
                              )}
                            </div>
