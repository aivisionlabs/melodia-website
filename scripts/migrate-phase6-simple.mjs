...
async function runMigrations() {
  ...
  try {
    ...
    // Add foreign key constraint for approved_lyrics_id (with error handling)
    try {
      await client`
        ALTER TABLE song_requests 
    ...
    await client`CREATE INDEX IF NOT EXISTS idx_lyrics_drafts_req ON lyrics_drafts(song_request_id);`;
    await client`CREATE INDEX IF NOT EXISTS idx_lyrics_drafts_status ON lyrics_drafts(status);`;
    await client`CREATE INDEX IF NOT EXISTS idx_lyrics_drafts_version ON lyrics_drafts(version DESC);`;
