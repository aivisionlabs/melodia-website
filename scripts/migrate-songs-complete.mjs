...
songsData.forEach((song, index) => {
  ...
  const duration = song.duration || 180.0;
  const createdAt = song.created_at || new Date().toISOString();
  ...
  if (song.timestamp_lyrics) {
    try {
      ...
      console.warn(
        `Warning: Could not parse timestamp_lyrics for ${song.title}`
      );
    }
  }
  ...
  if (
    song.timestamped_lyrics_api_responses &&
    song.timestamped_lyrics_api_responses !== "{}"
  ) {
    timestampLyricsApiResponse = escapeSql(
      song.timestamped_lyrics_api_responses
    );
  }
...
console.log(
  `   - ${songsData.length} songs with timestamped lyrics mapped to variant_timestamp_lyrics_processed`
);
