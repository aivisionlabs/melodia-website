...
async function checkSongRequests() {
  ...
  try {
    ...
    requests.forEach((request, index) => {
      ...
      console.log(`   Languages: ${request.languages?.join(', ') || 'Not specified'}`);
      console.log(`   Status: ${request.status}`);
      console.log(`   Lyrics Status: ${request.lyrics_status || 'NULL'} ${request.lyrics_status ? '✅' : '❌'}`);
      console.log(`   Created: ${new Date(request.created_at).toLocaleDateString()}`);
      console.log('');
    });
    ...
    if (nullLyricsStatus.length > 0) {
      console.log(`⚠️  ${nullLyricsStatus.length} request(s) still have NULL lyrics_status.`);
